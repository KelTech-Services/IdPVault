...
async function exLoadObjects(){
  ...
  try{
    ...
    tb.innerHTML = d.objects.map(o=>`<tr class="rowlink" onclick="exViewObject('${esc(o.object_id)}')">
      ...
      <td class="rowact" style="text-align:right">${_feat('identity') ? (o.tf_ok === false ? `<span title="Not exportable to Terraform: ${esc(o.tf_reason || 'the official provider cannot manage this object')}. It is still backed up." onclick="event.stopPropagation()"><button class="ghost" disabled>Terraform ${TIPI}</button></span> ` : `<button class="ghost" onclick="event.stopPropagation();exTfObject('${esc(o.object_id)}')" title="View this object as a Terraform resource block for the official ${esc(_tenants.find(x=>x.id===_ex.tenantId)?.provider || '')} provider, with an import block to adopt it into state">Terraform ${TIPI}</button> `) : ''}${canW && !inactive && o.status !== 'new' ? `<button class="ghost" onclick="event.stopPropagation();exRestoreObject('${esc(o.object_id)}')" title="Preview restoring this object from this backup (dry-run first, nothing is written until you apply)">Restore… ${TIPI}</button>` : ''}</td></tr>`).join('');

...
class OktaAdapter(ProviderAdapter):
    ...
    # App payload is whitelisted — Okta app objects carry heavy server-managed
    # structure; these are the writable parts. Secret material (client secrets,
    # SAML signing certs) is never in exports (IdP redacts it), so a restored app
    # comes back with REGENERATED credentials — integrations must be re-pointed.
    _APP_FIELDS = ("name", "label", "signOnMode", "settings", "visibility",
                   "profile", "accessibility")
    _INTERNAL_APPS = {"saasure", "okta_enduser", "okta_browser_plugin",
                      "okta_dashboard", "okta_flow", "okta_iga_reviews"}
    ...
    def push_object(self, resource_type: str, obj: dict, live: dict | None = None) -> tuple[str, str]:
        ...
        if resource_type == "groups":
            ...
            if obj.get("name") in self._INTERNAL_APPS:
                return ("skipped_system", str(obj.get("id", "")))  # Okta's own apps
            payload = {k: v for k, v in obj.items() if k in self._APP_FIELDS and v is not None}
            # recreate honoring the snapshot's active/inactive state
            params = {"activate": "true" if obj.get("status") == "ACTIVE" else "false"}
        else:
        ...
        with self._client() as c:
            ...
            r = self._rl.request(lambda: c.post(base, json=payload, params=params))
            if r.status_code >= 400:
                raise RuntimeError(f"POST {base} -> {r.status_code}: {r.text[:280]}")
            new_id = str(r.json().get("id", ""))
            if resource_type == "apps":
                return ("created_new_credentials", new_id)  # secrets regenerated by Okta
